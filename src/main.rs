...
mod heartbeat {
  ...
  #[async_trait]
  impl AsahiCoordinator<BotData> for HeartbeatLoop {
    ...
    async fn main_loop(
      ...
      let latency = self.ctx.runners.get(&self.ctx.shard_id).unwrap().0.latency;

      asahi::debug!("{latency:?}");

      self.probe.update_ws_status(true, latency).await;
      Ok(())
    }
  }
}
